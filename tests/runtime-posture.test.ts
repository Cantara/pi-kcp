...
describe("an ungoverned turn reaches the user by default", () => {
  ...
  it("says nothing when the turn was governed", async () => {
    ...
    await pi.fire("tool_call", { toolName: "read", input: {} }, dir);
    await pi.fire(
      "tool_result",
      { toolCallId: "t", toolName: "read", input: {}, content: [], isError: false },
      dir,
    );
...
describe("gate-failure posture", () => {
  ...
  it("announce: a broken gate does not block the tool call", async () => {
    ...
    await pi.fire("turn_start", { turnIndex: 1, timestamp: 0 }, dir);
    const decision = await pi.fire("tool_call", { toolName: "bash", input: {} }, dir);
  ...
  it("block: a broken gate refuses subsequent tool calls, with a reason", async () => {
    ...
    const first = await pi.fire("tool_call", { toolName: "bash", input: {} }, dir);
    ...
    const second = await pi.fire("tool_call", { toolName: "read", input: {} }, dir);
  ...
  it("block: a healthy gate still passes conformant calls", async () => {
    ...
    await pi.fire("turn_start", { turnIndex: 1, timestamp: 0 }, dir);
    expect(await pi.fire("tool_call", { toolName: "read", input: {} }, dir)).toBeUndefined();
    await rm(dir, { recursive: true, force: true });
  });
